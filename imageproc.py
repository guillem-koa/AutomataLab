def get_pocillos(img_bw):
    ...
    # Filtering out circles that are too close to the right or left edge of the picture!
    height, width = img_bw.shape
    circles = circles[(circles[:,0]/width <.95) & (circles[:,0]/width >.05)]

    return circles

def get_pocillos_yolo(img):
    import numpy as np
    from ultralytics import YOLO
    model = YOLO("well-detection-model.pt")

    # Circle detection
    results = model(img, conf = 0.4, iou = 0.7)
    circles = results[0].boxes.xywh.numpy()[:, :-1]
    radii = circles[:, -1]
    circles[:, -1] = 0.8 * radii / 2

    return circles.astype(int)


def get_positions(circles):
    import numpy as np
...
def draw_circles(img, circles):
    ...
    if circles is not None:
        ...
        for pt in circles:
            ...
            cv2.circle(canvas_img, (a, b), r, (0, 255, 0), 5)
            cv2.circle(canvas_img, (a, b), 1, (0, 0, 255), 5)
